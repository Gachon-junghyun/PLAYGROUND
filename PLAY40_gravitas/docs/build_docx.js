// gravitas_playbook.md → gravitas_playbook.docx
// 정보 밀도 보존(전 카드ID·플레이북 그대로) + 가독성(표 음영·콜아웃·목차).
// 자급: PLAY40/docs/node_modules 의 docx 사용 (다른 PLAY import 안 함).
const fs = require("fs");
...
const SRC = path.join(__dirname, "..", "gravitas_playbook.md");
const OUT = path.join(__dirname, "gravitas_playbook.docx");
...
const titlePage = [
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 200 },
    children: [new TextRun({ text: "무시받지 않는 위엄, 그 순간 쓰는 법", bold: true, font: FONT, size: 46, color: INK })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 600 },
    children: [new TextRun({ text: "8개 무시·무례 상황 플레이북", font: FONT, size: 30, color: ACCENT })] }),
  new Paragraph({ alignment: AlignmentType.CENTER,
    children: [new TextRun({ text: "유튜브 32편(KO16+EN16) → 추상 프레임(카드 133장) → 8개 무시·무례 상황 스트레스테스트", font: FONT, size: 19, color: MUTED })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 },
    children: [new TextRun({ text: "PLAY40_gravitas · 2026-06-06", font: FONT, size: 18, color: MUTED })] }),
...
const doc = new Document({
  creator: "PLAY40_gravitas",
  title: "무시받지 않는 위엄, 그 순간 쓰는 법",
  ...
  sections: [{
    ...
    headers: { default: new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT,
      children: [new TextRun({ text: "위엄·무게감 플레이북", font: FONT, size: 16, color: MUTED })] })] }) },
