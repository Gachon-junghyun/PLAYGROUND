...
- PLAY29는 **증권사 분석가 리포트** 기반 R4 카드 + R5 함수 신규 추출
...
| 측면 | PLAY13/28 (유튜브 자막) | PLAY29 (증권사 리포트) |
|---|---|---|
...
- `r5_thinking_functions.json` — 44 함수. PLAY29의 카드가 매칭 우선 시도.
...
### 통합 시점
PLAY29의 카드/함수가 어느 정도 쌓이면 (보수적으로 20카드/5함수 이상) `r5_thinking_functions.json`에 함수 후보 append. 이건 *수동 결정* — 자동 머지 금지.
...
# 예시 호출
Agent({
    "description": "PLAY29 broker R4 batch B1",
    "subagent_type": "general-purpose",
    "prompt": f"""
{open('PLAY29_broker_report_distill/prompts/broker_reverse_distill.md').read()}

---

## 이번 batch 호출 인자

- INPUT_LIST_FILE: PLAY29_broker_report_distill/data/batches/B1_kor_power.jsonl
- OUTPUT_CARDS_FILE: PLAY29_broker_report_distill/data/cards/B1_kor_power.jsonl
- OUTPUT_FUNCTIONS_FILE: PLAY29_broker_report_distill/data/cards/B1_kor_power.functions.jsonl
- CARD_ID_START: B001
- CARD_ID_END: B010
...
```jsonc
{
  ...
  "file_path": "PLAY29_broker_report_distill/data/inputs/rpt_001.txt",
  "page_count": 18,
  "size_bytes": 42183
}
```
...
- **로컬 ollama 기본**. 단 "지능 사용" 명시 시 Claude OK (PLAY29의 Agent는 그 경우).
