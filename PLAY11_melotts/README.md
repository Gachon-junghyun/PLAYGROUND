...
구조화된 스크립트(`슬라이드 N. ...` + `[화면]/[발화]/[핵심]` 블록)를 받아 슬라이드별 WAV로 출력. `[발화]` 블록만 합성한다.
...
출력: `out/slides/slide_NN.wav` + 파싱 결과 `out/slides/_parsed.txt`. 한 슬라이드 실패해도 다른 건 계속 진행.

**입력 형식 가정:**
- 슬라이드 헤더: `슬라이드 N.  <제목>` (decoration 라인 `───` 사이에 있음)
- 블록 태그: `[화면]`, `[발화]`, `[핵심]` 각각 줄 시작
- 합성 대상: 다음 태그(`[`)나 decoration 라인(`═══`/`───`)을 만날 때까지의 `[발화]` 이후 텍스트
