"""core.market — OHLCV fetcher (ticker → rows/CSV).

PLAY15_market_data_fetch 에서 승격. source 순위:
  1) yfinance (설치돼 있으면 import 시도 — lazy)
  2) KRX 일별시세 (urllib 직접 호출) — 한국 6자리 ticker 자동 분기
  3) deterministic 더미 (seed=ticker hash) — 위 둘 다 실패하거나 --source dummy

권유/판단 layer 아님. 단순 데이터 페치 + CSV 정규화.
yfinance 외 의존성 없음(전부 stdlib). yfinance 는 fetch_yfinance 안에서만 lazy import.
"""
...
def main(argv=None):
    p = argparse.ArgumentParser(description="OHLCV fetcher → CSV (core.market)")
